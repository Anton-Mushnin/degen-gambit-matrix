...
contract CommitRevealRandomness {
    ...
    /// @notice Inspect the random outcome of a committed move without executing the reveal
    /// @dev Allows previewing the random number that would be generated by reveal()
    /// @dev Same timing and validation checks as reveal(), but doesn't consume the commit
    ...
    /// @return randomNumber The random number that reveal() would generate
    function inspectOutcome(address player) external view returns (uint256 randomNumber) {
        uint256 currentBlock = _blockNumber();
        ...
        // Generate random number using entropy (same logic as reveal)
