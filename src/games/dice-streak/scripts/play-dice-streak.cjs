...
/**
 * DiceStreak Game Test Script
 *
 * This script tests the DiceStreak game functionality on XAI testnet.
 * It simulates playing multiple rounds of the dice game with automatic
 * bet placement and outcome resolution.
 *
 * REQUIREMENTS:
 * - Node.js installed
 * - .env file in project root with DEPLOYMENT_KEY set
 * - Testnet funds in the wallet address
 *
 * USAGE:
 *   From project root: node src/games/dice-streak/play-dice-streak.cjs
 *
 * ENVIRONMENT VARIABLES:
 * - DEPLOYMENT_KEY: Private key for the testnet wallet (required)
 *
 * NOTES:
 * - The script uses commit-reveal randomness based on future block hashes
 * - It automatically produces blocks when reveal blocks aren't ready
 * - Test runs for multiple rounds and tracks statistics
 * - Make sure the wallet has sufficient testnet funds
 */
