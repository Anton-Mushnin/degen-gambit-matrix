...
export interface CommitRevealSpinConfig {
  contractAddress: string;
  contractABI: readonly any[];
  spinFunctionName: string;
  spinArgs: any[];
  value?: bigint;
...
export interface CommitRevealSpinResult<T = readonly bigint[]> {
  description: string;
  outcome: T;
  receipt: string | null;
  prize?: string;
...
/**
 * Generic utility for handling commit-reveal pattern in spin-based games
 * Extracts the common logic from degenGambit spin function for reuse
 *
 * @example
 * ```typescript
 * // For any game using CommitRevealRandomness
 * const result = await commitRevealSpin({
 *   contractAddress: "0x...",
 *   contractABI: gameABI,
 *   spinFunctionName: 'spin', // or 'play', 'roll', etc.
 *   spinArgs: [boost], // game-specific arguments
 *   value: betAmount,
 *   account,
 *   client: thirdwebClient,
 *   publicClient,
 ...
 * });
 *
 * // Process game-specific outcome
 * const outcome = result.outcome;
 * // ... game logic here
 * ```
 */
export async function commitRevealSpin<T = readonly bigint[]>(
  config: CommitRevealSpinConfig
): Promise<CommitRevealSpinResult<T>> {
  const {
    contractAddress,
    contractABI,
    spinFunctionName,
  ...
  const tx = prepareContractCall({
    contract,
    method: spinFunctionName as any,
    params: spinArgs,
    value,
  });
