...
async function main() {
  ...
  // Update the play script with the deployed address
  const fs = require('fs');
  const playScript = fs.readFileSync('play-dice-streak.cjs', 'utf8');
  const updatedScript = playScript.replace(
    'const CONTRACT_ADDRESS = "0x5FbDB2315678afecb367f032d93F642f64180aa3";',
    `const CONTRACT_ADDRESS = "${diceStreak.address}";`
  );
  fs.writeFileSync('play-dice-streak.cjs', updatedScript);
  console.log("📝 Updated play script with deployed address");
}
